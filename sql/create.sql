-- ============================================================================
-- bigdata-adhoc 一期建表 DDL（含索引）
-- 来源：adhoc-dao/src/main/resources/db/schema.sql（已与 9 个 entity 类逐字段核对一致）
--       并入增量 ALTER：oss-schema-sync.sql / server-grpc-port.sql / job-log-path.sql
-- 目标库：adhoc（MySQL 8）。字符集 utf8mb4 / 引擎 InnoDB。
-- 执行：mysql -h<host> -P<port> -uroot -p<password> adhoc < create.sql
-- 幂等：所有表 CREATE TABLE IF NOT EXISTS，可重复执行。
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS adhoc_query_job (
  ...
  cancel_requested_time       DATETIME(3)      DEFAULT NULL COMMENT '取消请求时刻',
  ...
  submit_time                 DATETIME(3)      NOT NULL COMMENT 'Job 提交时间',
  validate_finish_time        DATETIME(3)      DEFAULT NULL COMMENT '前置校验完成时间',
  dispatch_time               DATETIME(3)      DEFAULT NULL COMMENT 'dispatchJob 下发时间（PENDING->RUNNING 触发点）',
  split_finish_time           DATETIME(3)      DEFAULT NULL COMMENT 'executor 拆分完成时间',
  start_time                  DATETIME(3)      DEFAULT NULL COMMENT '第一个 Task 开始执行时间',
  finish_time                 DATETIME(3)      DEFAULT NULL COMMENT 'Job 终态时间',
...
CREATE TABLE IF NOT EXISTS adhoc_query_task (
  ...
  enqueue_time                DATETIME(3)      DEFAULT NULL COMMENT 'Task 入队时间',
  start_time                  DATETIME(3)      DEFAULT NULL COMMENT 'Task 开始执行时间',
  fetch_start_time            DATETIME(3)      DEFAULT NULL COMMENT '开始拉结果时间',
  write_start_time            DATETIME(3)      DEFAULT NULL COMMENT '开始序列化+上传 OSS 时间',
  local_write_finish_time     DATETIME(3)      DEFAULT NULL COMMENT '结果序列化完成时间（内存）',
  oss_upload_time             DATETIME(3)      DEFAULT NULL COMMENT 'OSS 上传完成时间',
  finish_time                 DATETIME(3)      DEFAULT NULL COMMENT 'Task 终态时间',
...
CREATE TABLE IF NOT EXISTS adhoc_server_instance (
  ...
  start_time                  DATETIME(3)      DEFAULT NULL COMMENT '启动时间',
  version                     VARCHAR(32)   DEFAULT NULL COMMENT '版本号',
  heartbeat_time              DATETIME(3)      DEFAULT NULL COMMENT '最后心跳时间',
  ...
  memory_usage_pct            DOUBLE        DEFAULT NULL COMMENT 'JVM 堆使用率%',
  memory_used_mb              BIGINT        DEFAULT NULL COMMENT 'JVM 堆已用 MB',
  memory_max_mb               BIGINT        DEFAULT NULL COMMENT 'JVM 堆最大 MB',
  heap_committed_mb           BIGINT        DEFAULT NULL COMMENT 'JVM 堆已提交 MB',
  non_heap_used_mb            BIGINT        DEFAULT NULL COMMENT 'JVM 非堆已用 MB',
  non_heap_committed_mb       BIGINT        DEFAULT NULL COMMENT 'JVM 非堆已提交 MB',
  thread_count                INT           DEFAULT NULL COMMENT '线程数',
  daemon_thread_count         INT           DEFAULT NULL COMMENT 'daemon 线程数',
  gc_count                    BIGINT        DEFAULT NULL COMMENT 'GC 累计次数',
  gc_time_ms                  BIGINT        DEFAULT NULL COMMENT 'GC 累计耗时 ms',
  ...
  load_score                  DOUBLE        DEFAULT NULL COMMENT '综合负载评分(cpu+mem)',
  create_time                 DATETIME(3)      NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
  update_time                 DATETIME(3)      NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3) COMMENT '更新时间',
  PRIMARY KEY (id),
...
CREATE TABLE IF NOT EXISTS adhoc_executor_instance (
  ...
  version                     VARCHAR(32)   DEFAULT NULL COMMENT '版本号',
  heartbeat_time              DATETIME(3)      DEFAULT NULL COMMENT '最后心跳时间',
  last_down_time              DATETIME(3)      DEFAULT NULL COMMENT '最近一次 DOWN 时间',
...
-- 8.1 JVM/OS 指标采样历史（server 与 executor 各自心跳每 5s 写一条；server 侧定时清理超保留期数据）
CREATE TABLE IF NOT EXISTS adhoc_jvm_metric_sample (
  ...
  -- CPU / OS（com.sun.management.OperatingSystemMXBean）
  cpu_usage_pct               DOUBLE        DEFAULT NULL COMMENT 'JVM 进程 CPU%',
  system_cpu_usage_pct        DOUBLE        DEFAULT NULL COMMENT '系统整体 CPU%',
...
CREATE TABLE IF NOT EXISTS adhoc_file_node (
  ...
  parent_node_id              VARCHAR(64)   DEFAULT NULL COMMENT '父节点 ID，全局根为 NULL；用户根 parent=全局根',
  node_type                   VARCHAR(16)   NOT NULL COMMENT 'DIRECTORY/FILE/USER_ROOT(用户根)',
